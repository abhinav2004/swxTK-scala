...
def make_swig():
	print ("\n\t\033[1;37m--> Generating SWIG interfaces for wxApp.\033[0m")
	print ("\t" + SWIG + "-package wx.JNI -outdir src/wx/JNI src/wxapp_module.i")
	swigproc = os.system(SWIG + "-package wx.JNI -outdir src/wx/JNI src/wxapp_module.i")
	if swigproc is not 0:
		print ("Error! Aborting!")
		sys.exit(1)
def make_cpp():
	print ("\n\t\033[1;37m--> Compiling wxApp C++ library.\033[0m")
	print ("\t" + CXX + CXXOP + "src/java_wxapp.cpp -o obj/java_wxapp.o");
	cxxproc = os.system(CXX + CXXOP + "src/java_wxapp.cpp -o obj/java_wxapp.o");
	if cxxproc is not 0:
		print ("Error! Aborting!")
		sys.exit(1)
	
	print ("\n\t" + CXX + CXXOP + "src/wxapp_module_wrap.cxx -o obj/wxapp_module_wrap.o");
	cxxproc = os.system(CXX + CXXOP + "src/wxapp_module_wrap.cxx -o obj/wxapp_module_wrap.o");
	if cxxproc is not 0:
		print ("Error! Aborting!")
		sys.exit(1)
