...
def make_cpp():
	...
	print ("\n\t" + CXX + " -shared -fPIC `wx-config --cppflags` `wx-config --libs`" + "obj/wxframe_module_wrap.o obj/java_wxframe.o -o ./build/natives/libwxframe.so");
	cxxproc = os.system(CXX + " -shared -fPIC `wx-config --cppflags` `wx-config --libs`" + "obj/wxframe_module_wrap.o obj/java_wxframe.o -o ./build/natives/libwxframe.so");
	if cxxproc is not 0:
		print ("Error! Aborting!")
		sys.exit(1)
def make_java():
